...
def upload_and_transcribe_gemini(file_path: str) -> str:
    ...
    if requires_conversion:
        try:
            ...
            mime_type = "audio/wav"
            logging.info("File converted for upload")
    ...
    for key in gemini_rotator.get_order():
        try:
            ...
            with open(file_path, 'rb') as f:
                data = f.read()
                upload_response = requests.post(upload_url, headers=headers, data=data, timeout=REQUEST_TIMEOUT_GEMINI)
            if upload_response.status_code != 200:
                logging.warning("Gemini Upload failed, rotating key.")
            ...
            response = requests.post(generate_url, headers={"Content-Type": "application/json"}, json=payload, timeout=REQUEST_TIMEOUT_GEMINI)
            if response.status_code != 200:
                logging.warning("Gemini Transcription failed, rotating key.")
                gemini_rotator.mark_failure(key)
            ...
            logging.warning("Gemini transcription general error, rotating to next key")
            gemini_rotator.mark_failure(key)
            last_exc = e
    ...
    for key in gemini_rotator.get_order():
        try:
            ...
            if response.status_code == 200:
                ...
                logging.warning("Gemini key failed (403), rotating.")
                gemini_rotator.mark_failure(key)
                last_exc = f"403 Forbidden: {response.text}"
                continue
            else:
                logging.warning("Gemini API Error, rotating.")
                gemini_rotator.mark_failure(key)
                last_exc = f"HTTP {response.status_code}: {response.text}"
                continue
        except Exception as e:
            logging.warning("Gemini general error, rotating to next key")
            gemini_rotator.mark_failure(key)
            last_exc = e
            continue
...
def build_action_keyboard(text_length):
    buttons = []
    buttons.append([InlineKeyboardButton("⭐️ Translate", callback_data=f"translate_menu|")])
    if text_length > 1000:
...
WELCOME_MESSAGE = "👋 Salaam!\n• Send me a voice message, audio file, or video to transcribe it for free."
HELP_MESSAGE = f"/start - Show greeting\n/help - Help\nSend voice/audio file/video (up to {MAX_UPLOAD_MB}MB) to be transcribed"
...
async def send_join_prompt_to_target(client, uid: int, reply_target=None):
    ...
    text = "Please join my channel to continue"
    try:
...
async def language_callback_query(client, callback_query: CallbackQuery):
    ...
    try:
        ...
        await callback_query.answer("Language selection data is incorrect.", show_alert=True)
        return
    ...
    if origin == "trans":
        ...
        if not transcription_data:
            ...
            await callback_query.message.reply_text("Transcription information was not found.")
            return
...
async def choose_mode(client, message: Message):
    ...
    await message.reply_text("Choose the result mode:", reply_markup=keyboard)
...
async def mode_callback_query(client, callback_query: CallbackQuery):
    ...
    try:
        ...
        await callback_query.answer("Selection data is incorrect.", show_alert=True)
        return
    uid = callback_query.from_user.id
    user_mode[uid] = mode_name
    await callback_query.answer(f"Mode set: {mode_name}", show_alert=False)
    try:
        await callback_query.message.delete()
    except Exception:
        pass
...
async def handle_text(client, message: Message):
    ...
    if text in ["Split messages", "Text File", "💬 Split messages", "📄 Text File"]:
        ...
        user_mode[uid] = mode_name
        await message.reply_text(f"The result mode is: {mode_name}")
        return
...
async def action_callback_query(client, callback_query: CallbackQuery):
    ...
    if usage_count >= 1:
        await callback_query.answer(f"{action.capitalize()} is not available (used or expired)", show_alert=True)
        return
    ...
    instruction = ""
    if action == "summarize":
        instruction = "What is this report and what does it consist of? Summarize it in the original language without adding any introductory phrases or extra commentary."
    ...
    try:
        ...
        if len(processed_text) > MAX_MESSAGE_CHUNK:
            mode = get_user_mode(uid, "Text File")
            if mode == "Split messages":
                await client.send_message(chat_id, "Long result: I am sending it in parts.", reply_to_message_id=transcription_data["origin"])
        ...
        logging.error("Action callback error")
...
async def handle_media(client, message: Message):
    ...
    if duration is not None and duration > MAX_AUDIO_DURATION_SEC:
        ...
        await message.reply_text(f"The bot does not accept audio longer than {hours} hours. Please send a shorter one.")
        return
    ...
    if FFMPEG_BINARY is None:
        ...
        if file_ext and file_ext not in gemini_supported_extensions:
             await message.reply_text("FFMPEG is not available. Cannot convert video. Transcription will fail.")
             return
    ...
    if not text or text.startswith("Error:"):
        warning_text = text or "⚠️ Note: Make sure the audio is clear."
...
def set_wh():
    try:
        ...
        logging.error("Failed to set webhook for Bot 2")
        return "error", 500
...
def del_wh():
    try:
        ...
        logging.error("Failed to delete webhook for Bot 2")
        return "error", 500
...
def run_flask():
    ...
    try:
        if WEBHOOK_URL:
            telebot_bot.set_webhook(url=WEBHOOK_URL)
            logging.info("Relay bot webhook set successfully")
    except Exception:
        logging.error("Failed to set relay bot webhook on startup")
