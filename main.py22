...
def store_user_key(uid, key, reset_count=False):
    with lock:
        current = user_keys.get(uid, {})
        new_count = 0 if reset_count else current.get("count", 0)
        new_start = now_ts() if reset_count else current.get("window_start", now_ts())
        
        user_keys[uid] = {"key": key.strip(), "count": new_count, "window_start": new_start}
...
async def notify_admin(client, msg):
    try:
...
def format_user_contact(user):
    ...
    name = (getattr(user, "first_name", "") or "") + ((" " + getattr(user, "last_name", "")) if getattr(user, "last_name", "") else "")
    if uname:
        return f"@{uname}"
    if phone:
        return str(phone)
    return name or "No username/phone"

@app.on_message(filters.command(['start', 'help']))
async def send_welcome(client, message):
    uid = message.from_user.id
    if uid in user_keys or (users_col is not None and users_col.find_one({"uid": uid})):
        welcome_text = (
            "👋 Welcome back!\n"
...
async def set_key_cmd(client, message):
    args = message.text.split(maxsplit=1)
    if len(args) < 2:
        await message.reply_text("Usage: /set YOUR_GEMINI_KEY")
        return
    key = args[1].strip()
    if not is_gemini_key(key):
        await message.reply_text("❌ This does not look like a valid Gemini key. Please try again.")
        return
    uid = message.from_user.id
    store_user_key(uid, key, reset_count=True)
    contact = format_user_contact(message.from_user)
    await notify_admin(client, f"🔔 User Update:\nID: {uid}\nName: {message.from_user.first_name}\nContact: {contact}\nAction: Changed API Key")
    await message.reply_text("☑️ Key updated successfully!\nDaily count has been reset.\nYou can now make 19 requests today.")

@app.on_message(filters.text & ~filters.command(["start", "help", "set", "getcount", "removekey"]))
async def text_handler(client, message):
    uid = message.from_user.id
    if user_awaiting_key.get(uid):
        key = message.text.strip()
        if not is_gemini_key(key):
            await message.reply_text("❌ Not a valid Gemini key. Please try again or create one at Google AI Studio.")
            return
        store_user_key(uid, key, reset_count=True)
        contact = format_user_contact(message.from_user)
        await notify_admin(client, f"🟢 New User Registered:\nID: {uid}\nName: {message.from_user.first_name}\nContact: {contact}\nKey: {key[:10]}...")
        await message.reply_text("☑️ Key registered successfully!\nYour daily limit is 19 requests.\nNow send me audio or video to transcribe.")
    else:
        data = user_transcriptions.get(message.chat.id, {}).get(message.reply_to_message_id)
        if data:
             return
        await client.send_chat_action(message.chat.id, enums.ChatAction.TYPING)
        try:
            res = ask_gemini(message.text, "You are a helpful assistant.", uid)
