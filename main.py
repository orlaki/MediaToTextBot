...
MAX_CHUNK_SEC = int(os.environ.get("MAX_CHUNK_SEC", "60"))
...
def handle_media(message):
    ...
    try:
        ...
        if not lang:
            ...
            bot.reply_to(message, "Select the language spoken in your audio or video:", reply_markup=kb)
            return
        text = transcribe_file(file_path, language=lang)
        if not text:
...
def transcribe_file(file_path, language=None):
    ...
    try:
        ...
        chunks = []
        length_ms = len(audio)
        for start in range(0, length_ms, chunk_ms):
            end = min(start + chunk_ms, length_ms)
            chunk = audio[start:end]
            chunk_name = os.path.join(tmpdir, f"chunk_{start}.wav")
