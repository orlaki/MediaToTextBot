...
def execute_assembly_action(action_callback):
    last_exc = None
    total = len(assembly_rotator.keys) or 1
...
def build_action_keyboard(text_len):
    btns = []
    btns.append([InlineKeyboardButton("Summarize", callback_data="summarize_menu|")])
    if text_len > 1000:
        btns.append([InlineKeyboardButton("Download full file", callback_data="download_full|")])
...
@bot.callback_query_handler(func=lambda c: c.data.startswith('summarize_menu|') or c.data.startswith('download_full|'))
def action_cb(call):
    action = call.data.split("|")[0]
    if action == "summarize_menu":
        try:
            ...
            try:
                ...
                pass
    elif action == "download_full":
        try:
            bot.answer_callback_query(call.id, "This feature isn't implemented here.")
        except:
            pass
