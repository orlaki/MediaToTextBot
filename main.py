...
def gemini_api_call(endpoint, payload, key, headers=None):
    ...
    resp = requests.post(url, headers=headers or {"Content-Type": "application/json"}, json=payload, timeout=REQUEST_TIMEOUT)
...
def lang_cb(call):
    ...
    if origin == "file":
        ...
        if not pending:
            bot.send_message(chat_id, "No pending file found. Send a file to transcribe.")
            return
