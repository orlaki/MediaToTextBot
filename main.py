...
def choose_mode(message):
    if ensure_joined(message):
        ...
        bot.reply_to(message, "How do you want me to send you text that is longer than Telegram’s 4096-character limit?", reply_markup=kb)
...
def mode_cb(call):
    ...
    try:
        bot.edit_message_text(f"Output mode: {mode}", call.message.chat.id, call.message.message_id, reply_markup=None)
    except: pass
