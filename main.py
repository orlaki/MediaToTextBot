...
ASSEMBLYAI_API_KEYS = os.environ.get("ASSEMBLYAI_API_KEYS", "")
...
assemblyai_rotator = KeyRotator(ASSEMBLYAI_API_KEYS)
...
def upload_and_transcribe_assemblyai(file_path: str) -> str:
    key = assemblyai_rotator.get_key()
    if not key:
    ...
    try:
        with open(file_path, "rb") as f:
            response = requests.post("https://api.assemblyai.com/v2/upload", headers=headers, data=f, timeout=300)
        ...
        response = requests.post("https://api.assemblyai.com/v2/transcript", json=transcript_json, headers=headers, timeout=60)
        if not response.ok:
            raise RuntimeError(f"AssemblyAI Transcribe failed: {response.text}")
            
        transcript_id = response.json()["id"]
        
        while True:
            response = requests.get(f"https://api.assemblyai.com/v2/transcript/{transcript_id}", headers=headers, timeout=60)
            if not response.ok:
                raise RuntimeError(f"AssemblyAI Polling failed: {response.text}")
                
            status = response.json()["status"]
            if status == "completed":
                return response.json()["text"]
            elif status == "error":
                raise RuntimeError("AssemblyAI processing error")
            
            time.sleep(3)
            
    except Exception as e:
        assemblyai_rotator.mark_failure(key)
        raise e
    else:
        assemblyai_rotator.mark_success(key)
...
def handle_media(message):
    ...
    try:
        ...
        try:
            ...
            logging.warning(f"Gemini failed, trying AssemblyAI. Error: {e}")
            if ASSEMBLYAI_API_KEYS:
                bot.send_chat_action(message.chat.id, 'typing')
                text = upload_and_transcribe_assemblyai(file_path)
            else:
                raise e
