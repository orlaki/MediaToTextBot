...
HUGGINGFACE_WHISPER_MODEL = os.environ.get("HUGGINGFACE_WHISPER_MODEL", "openai/whisper-tiny")
OPENAI_API_KEY = os.environ.get("OPENAI_API_KEY", "")
WHISPER_MODEL = os.environ.get("WHISPER_MODEL", "whisper-1")
WHISPER_BACKEND = os.environ.get("WHISPER_BACKEND", "hf" if HUGGINGFACE_API_KEY else "openai")
...
def transcribe_with_huggingface(file_path, model, language=None):
    ...
    resp = requests.post(url, headers=headers, files=files, params=params, timeout=REQUEST_TIMEOUT)
    ...
    if isinstance(data, dict):
        ...
        if "result" in data and isinstance(data.get("result"), dict) and "text" in data.get("result"):
            return data["result"]["text"]
    if isinstance(data, str):
        return data
    raise RuntimeError("Unexpected HuggingFace response")

def transcribe_with_openai(file_path, model, language=None):
...
def upload_and_transcribe_whisper(file_path, language=None):
    if WHISPER_BACKEND == "hf" and HUGGINGFACE_API_KEY:
        return transcribe_with_huggingface(file_path, HUGGINGFACE_WHISPER_MODEL, language=language)
    if WHISPER_BACKEND == "openai" and OPENAI_API_KEY:
        return transcribe_with_openai(file_path, WHISPER_MODEL, language=language)
...
def lang_cb(call):
    ...
    try:
        text = upload_and_transcribe_whisper(file_path, language=code)
        if not text:
...
def handle_media(message):
    ...
    try:
        ...
        if not lang:
            ...
            bot.reply_to(message, "Select the language spoken in your audio or video:", reply_markup=kb)
            return
        text = upload_and_transcribe_whisper(file_path, language=lang)
        if not text:
