...
WHISPER_CPP_PATH = os.environ.get("WHISPER_CPP_PATH", "./whisper.cpp/main")
WHISPER_MODEL_PATH = os.environ.get("WHISPER_MODEL_PATH", "./whisper.cpp/models/ggml-base.en.bin")
...
def transcribe_with_whisper_cpp(file_path, language_code):
    if not os.path.exists(WHISPER_CPP_PATH) or not os.path.exists(WHISPER_MODEL_PATH):
        raise RuntimeError("Whisper.cpp binary or model not found. Check configuration.")
    
    wav_path = file_path + ".wav"
    preprocess_audio(file_path, wav_path)
    ...
    try:
        subprocess.run(command, capture_output=True, text=True, check=True, timeout=REQUEST_TIMEOUT)
        
        output_file = wav_path + ".txt"
        if not os.path.exists(output_file):
             raise RuntimeError("Whisper.cpp output file not created.")
...
def lang_cb(call):
    ...
    try:
        text = transcribe_with_whisper_cpp(file_path, language=code)
        if not text:
...
def handle_media(message):
    ...
    try:
        ...
        if not lang:
            ...
            bot.reply_to(message, "Select the language spoken in your audio or video:", reply_markup=kb)
            return
        text = transcribe_with_whisper_cpp(file_path, language=lang)
        if not text:
