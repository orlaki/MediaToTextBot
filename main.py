...
# --- GEMINI REST API FUNCTIONS (FIXED WITH RAW UPLOAD) ---
...
def upload_and_transcribe_gemini(file_path: str, lang_code: str = "en") -> str:
    ...
    for key in gemini_rotator.get_order():
        try:
            # --- 1. UPLOAD (RAW REST API METHOD) ---
            # NOTE: We use /upload/v1beta/files NOT /v1beta/files
            ...
            logging.info(f"Attempting RAW file upload: {file_path} ({file_size} bytes) with key {key[:4]}...")
            ...
            with open(file_path, 'rb') as f:
                ...
                upload_response = requests.post(
                    upload_url,
                    headers=headers,
                    data=data,  # Send raw bytes as body
            ...
            # With raw upload, the file object is usually directly in the root or under 'file'
            if "file" in upload_data and "name" in upload_data["file"]:
                uploaded_file_name = upload_data["file"]["name"]
            elif "name" in upload_data:
                uploaded_file_name = upload_data["name"]
            else:
                logging.error(f"JSON Structure unknown: {upload_data}")
                raise RuntimeError(f"Uploaded file name missing from response JSON: {upload_data}")

            logging.info(f"File uploaded successfully: {uploaded_file_name}")
            ...
            payload = {
                "contents": [{
                    "parts": [
                        {"fileData": {"mimeType": mime_type, "fileUri": uploaded_file_name}},
                        {"text": prompt}
                    ]
                }]
            }
            ...
            if response.status_code != 200:
                ...
                try:
                    delete_url = f"https://generativelanguage.googleapis.com/v1beta/files/{uploaded_file_name}?key={key}"
