...
def handle_media(message):
    ...
    file_path = os.path.join(DOWNLOADS_DIR, f"temp_{message.id}_{media.file_unique_id}{ext}")
    try:
        downloaded = bot.download_file(file_info.file_path)
        with open(file_path, 'wb') as f: f.write(downloaded)
        text_exts = ['.txt', '.text', '.srt', '.md', '.json', '.csv']
        if message.document and ext in text_exts:
            with open(file_path, 'rb') as doc:
                bot.send_document(message.chat.id, doc, caption="Open this file and copy the text inside 👍", reply_to_message_id=message.id)
            return
...
def send_long_text(chat_id, text, reply_id, uid, action="Transcript"):
    ...
    if len(text) > MAX_MESSAGE_CHUNK:
        if mode == "Split messages":
            ...
            if action == "Transcript" or action.lower().startswith("translate to"):
                caption = "Open this file and copy the text inside 👍"
            else:
                caption = f"{action} Result"
