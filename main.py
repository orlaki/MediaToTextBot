...
def transcribe_media_gemini(file_url, mime_type, language_code):
    if not gemini_rotator.keys:
...
def lang_cb(call):
    ...
    try:
        text = transcribe_media_gemini(file_url, mime_type, code)
        if not text:
...
def handle_media(message):
    ...
    try:
        ...
        if not lang:
            ...
            bot.reply_to(message, "Select the language spoken in your audio or video:", reply_markup=kb)
            return
        text = transcribe_media_gemini(telegram_file_url, mime_type, lang)
        if not text:
