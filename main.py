...
def execute_gemini_action(action_callback):
    """
    Tries all keys and all models indefinitely until success.
    Waits 2 seconds between full retry cycles to avoid tight loops.
    """
    while True:
        last_exc = None
        ...
        for model in GEMINI_MODEL_FALLBACKS:
            ...
            # Try every key for the current model
            for _ in range(keys_to_try):
                key = gemini_rotator.get_key()
                if not key:
                    # Should generally not happen if keys_count > 0
        ...
        # Log error, wait a bit, then RESTART the loop from the top.
