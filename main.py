...
def _is_retryable_api_error(e):
    try:
        msg = str(e)
        if "503" in msg or "UNAVAILABLE" in msg or "overloaded" in msg or "temporarily" in msg:
            return True
    except:
        pass
    try:
        code = getattr(e, "status", None) or getattr(e, "code", None) or getattr(e, "status_code", None)
        if code and int(code) >= 500 and int(code) < 600:
            return True
    except:
        pass
    return False

def _generate_with_retries(api_key, contents, uid, initial_model=None, max_attempts_primary=3, max_attempts_fallback=2, base_backoff=2.0):
    client = genai.Client(api_key=api_key)
    model_sequence = []
    if initial_model:
        model_sequence.append(initial_model)
    else:
        model_sequence.append(get_current_model(uid))
    if model_sequence[0] == PRIMARY_MODEL:
        model_sequence.append(FALLBACK_MODEL)
    else:
        model_sequence.append(PRIMARY_MODEL)
    last_exc = None
    for idx, model in enumerate(model_sequence):
        attempts = max_attempts_primary if model == PRIMARY_MODEL else max_attempts_fallback
...
def upload_and_transcribe_gemini(file_path: str, key: str, uid: int) -> str:
    ...
    try:
        ...
        response = _generate_with_retries(key, [prompt, uploaded_file], uid, initial_model=current_model)
...
def ask_gemini(text, instruction, key, uid):
    try:
        ...
        current_model = get_current_model(uid)
        response = _generate_with_retries(key, [prompt], uid, initial_model=current_model)
        return response.text
    except APIError as e:
        raise RuntimeError(f"Gemini API Error: {e}")
    except Exception as e:
...
async def handle_media(client, message):
    ...
    try:
        ...
        if not opt_path:
            text = await asyncio.get_event_loop().run_in_executor(None, upload_and_transcribe_gemini, d_path, user_key, message.from_user.id)
        else:
            try:
                ...
                pass
            text = await asyncio.get_event_loop().run_in_executor(None, upload_and_transcribe_gemini, opt_path, user_key, message.from_user.id)
        if not text:
        ...
        if "503" in msg or "UNAVAILABLE" in msg or "overloaded" in msg:
            await message.reply_text("⚠️ Gemini servers are busy. I retried automatically. If this continues, please try again later or wait a minute.", quote=True)
        else:
