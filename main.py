...
def build_action_keyboard(text_length):
    buttons = []
    # Badhanka Nadiifinta (Clean transcript) hadda jira ka dhig mid Turjumid (Translate) ah.
    buttons.append([InlineKeyboardButton("Translate", callback_data=f"translate|")])
    buttons.append([InlineKeyboardButton("Clean transcript", callback_data=f"clean|")])
    if text_length > 1000:
        buttons.append([InlineKeyboardButton("Summarize", callback_data=f"summarize|")])
    return InlineKeyboardMarkup(buttons)

def build_language_selection_keyboard(prefix="translate_lang"):
    buttons, row = [], []
    for i, (label, code) in enumerate(LANGS, 1):
        row.append(InlineKeyboardButton(label, callback_data=f"{prefix}|{code}|{label}"))
        if i % 3 == 0:
            buttons.append(row)
            row = []
    if row:
...
WELCOME_MESSAGE = "Hello!\n• Send a voice message, audio file, or video to transcribe for free"
...
# **********************************************
# KA-SAARIDDA COMMAND-KA /lang
# Command-ka /lang waa laga saaray halkan.
# **********************************************

# Fiiro Gaar Ah: In kasta oo shaqada 'language_callback_query' ay jirtay, haddana lama isticmaali doono 
# xaaladaha 'lang' ama 'start' maadaama /lang la saaray. Waxaa loo hayaa oo kaliya
# shaqada cusub ee "Translate" badhanka.
# Waxaa la beddelay 'lang|' una beddelay 'translate_lang|'
@app.on_callback_query(filters.regex(r"^lang\|")) # Lama isticmaali doono laakin waa loo hayaa in lagu sii hayo luuqada hore
...
async def translate_language_callback_query(client, callback_query: CallbackQuery):
    if not await ensure_joined(client, callback_query):
        return
    try:
        parts = callback_query.data.split("|")
        _, target_code, target_label = parts[:3]
    except Exception:
        await callback_query.answer("Invalid translation language selection data.", show_alert=True)
        return
    ...
    # 2. Marka isticmaaluhu doorto luqadda (tusaale: English, Spanish, iwm.)
    # • Bot-ku waa inuu qabtaa callback-ga luqaddaas la doortay.
    # • Bot-ku waa inuu tirtiraa keyboard-ka luuqadaha iyo fariinta ay wadatay.
    ...
    reply_to_msg_id = transcription_data["origin"]
    
    try:
        await callback_query.message.edit_text(f"Translating to **{target_label}**...", reply_markup=None)
    except Exception:
        pass
    
    await callback_query.answer(f"Translating to {target_label}...", show_alert=False)
    await client.send_chat_action(chat_id, ChatAction.TYPING)
    
    # 3. Bot-ku waa inuu isla markiiba bilaabaa Turjumidda (Translation Step) ee transcript-kii hore.
    instruction = f"Translate the following text into **{target_label}**. Do not add any introductory or concluding phrases, just provide the translation."
    key = f"{chat_id}|{reply_to_msg_id}|translate_{target_code}"
    usage_count = action_usage.get(key, 0)
    
    if usage_count >= 1:
        await client.send_message(chat_id, f"You already translated this message to **{target_label}**.", reply_to_message_id=reply_to_msg_id)
        return
    ...
    try:
        ...
        if translated_text.startswith("Error:"):
            await client.send_message(chat_id, f"Translation Error: {translated_text}", reply_to_message_id=reply_to_msg_id)
            return

        action_usage[key] = usage_count + 1
        
        # Hadda oo turjumaaddii la dhammeeyay, aan soo celinno fariinta asalka ah
        # iyadoo lagu beddelayo badhanka **Translate** badhan **✅ Translate** ah.
        keyboard_buttons = build_action_keyboard(len(original_text)).inline_keyboard
        new_buttons = []
        for row in keyboard_buttons:
            new_row = []
            for button in row:
                if button.callback_data and button.callback_data.startswith("translate|"):
                    new_row.append(InlineKeyboardButton(f"✅ Translate", callback_data="used_translate"))
                else:
                    new_row.append(button)
            new_buttons.append(new_row)
        
        # Ku soo celi fariintii hore oo la tafatiray (haddii ay wali jirto)
        # Laakin halkan, ma rabno inaan tirtirno fariintii soo turjuntay
        # Waxaan ka dhigaynaa fariintii hore inay ku celiso transcript-kii.
        # Maadaama fariinta "Translating to..." la tafatiray, aan dib u tafatirno si ay u noqoto transcript-kii hore.
        # Fiiro gaar ah: Mararka qaar waa adagtahay in la ogaado transcript-kii asalka ah
        # marka fariintii la tirtiro ama la tafatiro. Waxaan u baahanahay inaan 
        # dib u soo celinno badhannada asalka ah.
        
        # Ku soo celi fariinta asalka ah badhamadii asalka ahaa
        try:
            await callback_query.message.edit_text(
                f"**Translated to {target_label}:**\n\n{translated_text}", 
                reply_markup=InlineKeyboardMarkup(new_buttons)
            )
        except Exception as e:
            logging.error(f"Error re-editing message: {e}")
            await client.send_message(chat_id, f"**Translated to {target_label}:**\n\n{translated_text}", reply_to_message_id=reply_to_msg_id)


        # Haddii ay aad u dheer tahay, u dir file ahaan ama qayb-qayb
        if len(translated_text) > MAX_MESSAGE_CHUNK:
            mode = get_user_mode(uid, "Text File")
            if mode == "Split messages":
                await client.send_message(chat_id, "Long result: sending in parts.", reply_to_message_id=reply_to_msg_id)
                for part in [translated_text[i:i+MAX_MESSAGE_CHUNK] for i in range(0, len(translated_text), MAX_MESSAGE_CHUNK)]:
                    await client.send_chat_action(chat_id, ChatAction.TYPING)
                    await client.send_message(chat_id, part, reply_to_message_id=reply_to_msg_id)
            else:
                file_name = os.path.join(DOWNLOADS_DIR, f"Translation_{target_code}.txt")
                with open(file_name, "w", encoding="utf-8") as f:
                    f.write(translated_text)
                await client.send_chat_action(chat_id, ChatAction.UPLOAD_DOCUMENT)
                await client.send_document(chat_id, file_name, caption=f"Translation to {target_label}", reply_to_message_id=reply_to_msg_id)
                os.remove(file_name)
        else:
             # U dir turjumaaddii, laakiin u isticmaal reply si loogu xiro fariintii asalka ahayd.
            await client.send_message(chat_id, f"**Translated to {target_label}:**\n\n{translated_text}", reply_to_message_id=reply_to_msg_id)

    except RuntimeError as e:
        await callback_query.message.reply_text(f"Translation Error: {e}", reply_to_message_id=reply_to_msg_id)
    except Exception as e:
        logging.error(f"Translation callback error: {e}")
        await client.send_message(chat_id, f"Unexpected Translation error: {e}", reply_to_message_id=reply_to_msg_id)
...
@app.on_callback_query(filters.regex(r"^(translate|clean|summarize)\|"))
async def action_callback_query(client, callback_query: CallbackQuery):
    ...
    if action == "translate":
        # 1. Bot-ku waa inuu soo muujiyaa keyboard-ka luuqadaha (Language Selection Keyboard) oo hadda jira.
        keyboard = build_language_selection_keyboard(prefix="translate_lang")
        try:
            await callback_query.message.edit_text("Choose a language to translate the transcript to:", reply_markup=keyboard)
        except Exception:
            await client.send_message(chat_id, "Choose a language to translate the transcript to:", reply_markup=keyboard, reply_to_message_id=reply_to_msg_id)
        await callback_query.answer("Choose target language...", show_alert=False)
        return

    # Hadii aysan ahayn "translate", waxaan sii wadeynaa "clean" ama "summarize"
    ...
    instruction = ""
    if action == "clean":
        instruction = "Clean up the provided text. Fix punctuation, capitalization, and grammatical errors. Ensure the output is a professional, polished version of the original text. Do not add any introductory or concluding phrases."
    ...
    try:
        ...
        if processed_text.startswith("Error:"):
            await client.send_message(chat_id, f"Error: {processed_text}", reply_to_message_id=reply_to_msg_id)
            return
        action_usage[key] = usage_count + 1
        keyboard = build_action_keyboard(len(original_text))
        new_buttons = []
        for row in keyboard.inline_keyboard:
            new_row = []
            for button in row:
                if button.callback_data and button.callback_data.startswith(f"{action}|"):
                    new_row.append(InlineKeyboardButton(f"✅ {button.text}", callback_data="used_action"))
                else:
                    new_row.append(button)
            new_buttons.append(new_row)
        try:
        ...
        # Ku soo celi jawaabta
        if action == "clean":
            response_caption = "**Cleaned Transcript:**"
        elif action == "summarize":
            response_caption = "**Summary:**"
        else:
            response_caption = ""

        if len(final_response) > MAX_MESSAGE_CHUNK:
            mode = get_user_mode(uid, "Text File")
            if mode == "Split messages":
                await client.send_message(chat_id, f"Long result: sending in parts.\n{response_caption}", reply_to_message_id=reply_to_msg_id)
                for part in [final_response[i:i+MAX_MESSAGE_CHUNK] for i in range(0, len(final_response), MAX_MESSAGE_CHUNK)]:
                    await client.send_chat_action(chat_id, ChatAction.TYPING)
                    await client.send_message(chat_id, part, reply_to_message_id=reply_to_msg_id)
            else:
                file_name = os.path.join(DOWNLOADS_DIR, f"{action}_result.txt")
                with open(file_name, "w", encoding="utf-8") as f:
                    f.write(final_response)
                await client.send_chat_action(chat_id, ChatAction.UPLOAD_DOCUMENT)
                await client.send_document(chat_id, file_name, caption=response_caption, reply_to_message_id=reply_to_msg_id)
                os.remove(file_name)
        else:
            await client.send_message(chat_id, f"{response_caption}\n\n{final_response}", reply_to_message_id=reply_to_msg_id)
            
    except RuntimeError as e:
        await client.send_message(chat_id, f"Error: {e}", reply_to_message_id=reply_to_msg_id)
    except Exception as e:
        logging.error(f"Action callback error: {e}")
        await client.send_message(chat_id, f"Unexpected error: {e}", reply_to_message_id=reply_to_msg_id)
...
async def handle_media(client, message: Message):
    ...
    # Doorashada luqadda (Language Selection) mar dambe lama soo bandhigi doono halkan.
    # Isticmaaluhu waxa uu ka dooran karaa luqadda hadda ka dib badhanka Turjumidda.
    ...
    # Luqadda Transcription-ka waxaa loo isticmaali doonaa default 'en' haddii aan la dejin.
    ...
    if not text or text.startswith("Error:"):
        await message.reply_text(text or "Warning: Check audio clarity or language selection.", reply_to_message_id=message.id)
        return
        
    reply_msg_id = message.id
    sent_message = None
    
    # Ku soo dir Transcription-ka
    if len(text) > MAX_MESSAGE_CHUNK:
        if mode == "Split messages":
            await message.reply_text("Long result: sending in parts.", reply_to_message_id=reply_msg_id)
            for part in [text[i:i+MAX_MESSAGE_CHUNK] for i in range(0, len(text), MAX_MESSAGE_CHUNK)]:
                await client.send_chat_action(message.chat.id, ChatAction.TYPING)
                sent_message = await client.send_message(message.chat.id, part, reply_to_message_id=reply_msg_id)
        else:
    ...
    if sent_message:
        try:
            ...
            # U isticmaal 'clean' badhanka asalka ah. 'translate' hadda waa shaqo cusub oo loo marayo badhanka hore ee 'clean'
            action_usage[f"{sent_message.chat.id}|{sent_message.id}|clean"] = 0 
            action_usage[f"{sent_message.chat.id}|{sent_message.id}|translate"] = 0
            if len(text) > 1000:
