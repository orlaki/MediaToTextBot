...
def summarize_youtube_link(url, key, uid):
    try:
        client = genai.Client(api_key=key)
        prompt = f"Please provide a detailed summary of this YouTube video: {url}. Explain the main points and key takeaways clearly in the language of the video. Return only the summary."
        current_model = get_current_model(uid)
        response = client.models.generate_content(model=current_model, contents=[prompt])
        return response.text
    except Exception as e:
        raise RuntimeError(f"YouTube Summary Error: {e}")
...
async def send_welcome(client, message):
    if await ensure_joined(client, message):
        welcome_text = "👋 Salaam Laki!\n• Send me\n• YouTube Link (for summary)\n• Voice/Audio/Video (for transcription)"
...
async def handle_youtube(client, message):
    ...
    msg = await message.reply_text("Summarizing YouTube video... Please wait ⏳", quote=True)
    try:
        loop = asyncio.get_event_loop()
        summary = await loop.run_in_executor(None, summarize_youtube_link, url, user_key, message.from_user.id)
        await msg.delete()
        await send_long_text(client, message.chat.id, summary, message.id, message.from_user.id, "YouTube Summary")
    except Exception as e:
...
async def set_key_plain(client, message):
    ...
    await message.reply_text("Okay Gemini key saved! Send me audio or YouTube link 👍", quote=True)
...
async def summopt_cb(client, call):
    ...
    prompt = ""
    if style == "Short":
        prompt = "Summarize this text in 1-2 concise sentences. Return only summary."
    elif style == "Detailed":
        prompt = "Summarize this text in a detailed paragraph. Return only summary."
    else:
        prompt = "Summarize this text as a bulleted list. Return only summary."
...
async def process_text_action(client, call, origin_msg_id, log_action, prompt_instr):
    ...
    try:
        loop = asyncio.get_event_loop()
        res = await loop.run_in_executor(None, ask_gemini, data["text"], prompt_instr, user_key, call.from_user.id)
...
@flask_app.route("/")
def index():
    return f"Bot is Online. Uptime: {int(time.time() - start_time)}s"
