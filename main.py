...
async def send_join_prompt_to_target(client, uid: int, reply_target=None):
    ...
    text = "Join my channel, it's mandatory 🤡"
    try:
...
async def start(client, message: Message):
    ...
    await message.reply_text("Choose your file language for transcription using the buttons below:", reply_markup=keyboard)
...
async def action_callback_query(client, callback_query: CallbackQuery):
    ...
    if not transcription_data:
        await callback_query.message.reply_text("Transcription not found. Please resend the message.")
        return
    original_text = transcription_data["text"]
    key = f"{chat_id}|{message_id}|{action}"
    usage_count = action_usage.get(key, 0)
    if usage_count >= 1:
        await callback_query.message.reply_text(f"You already used {action} on this message.")
        return
    ...
    try:
        ...
        if processed_text.startswith("Error:"):
            await callback_query.message.reply_text(f"Error: {processed_text}", reply_to_message_id=transcription_data["origin"])
            return
        ...
        for row in keyboard.inline_keyboard:
            ...
            for button in row:
                if button.callback_data and button.callback_data.startswith(f"{action}|"):
                    new_row.append(InlineKeyboardButton(f"✅ {button.text}", callback_data="used"))
                else:
        ...
        if len(final_response) > MAX_MESSAGE_CHUNK:
            ...
            if mode == "Split messages":
                await callback_query.message.reply_text("Long result: sending in parts.")
                ...
                await client.send_document(chat_id, file_name, caption="Transcript", reply_to_message_id=transcription_data["origin"])
                os.remove(file_name)
        else:
            await client.send_message(chat_id, final_response, reply_to_message_id=transcription_data["origin"])
    except RuntimeError as e:
