...
def optimize_audio(input_path):
    output_path = os.path.join(DOWNLOADS_DIR, f"base_opt_{int(time.time())}.mp3")
    try:
        command = [
            "ffmpeg", "-i", input_path,
            "-vn",
...
def upload_and_transcribe_gemini(file_path: str, key: str, uid: int) -> str:
    ...
    try:
        ...
        uploaded_file = client.files.upload(file=optimized_path)
        ...
        if uploaded_file:
            try:
                ...
                pass
        for path in [file_path, optimized_path]:
...
async def summopt_cb(client, call):
    ...
    prompt = ""
    if style == "Short":
        prompt = "Summarize this text in the original language in 1-2 concise sentences."
    elif style == "Detailed":
        prompt = "Summarize this text in the original language in a detailed paragraph."
    else:
        prompt = "Summarize this text in the original language as a bulleted list."
...
async def process_text_action(client, call, origin_msg_id, log_action, prompt_instr):
    ...
    if not user_key:
        await call.answer("Gemini key not set", show_alert=True)
        return
...
async def handle_media(client, message):
    ...
    if getattr(media, "file_size", 0) > MAX_UPLOAD_SIZE:
        await message.reply_text(f"File too large. Max {MAX_UPLOAD_MB}MB.", quote=True)
        return
    user_key = get_user_key_db(message.from_user.id)
    if not user_key:
        await message.reply_text("Please send your Gemini API key first.", quote=True)
        return
    await client.send_chat_action(message.chat.id, enums.ChatAction.TYPING)
    temp_name = f"raw_{message.id}_{media.file_unique_id}"
    file_path = os.path.join(DOWNLOADS_DIR, temp_name)
    try:
    ...
    if len(text) > MAX_MESSAGE_CHUNK:
        if mode == "Split messages":
            ...
            with open(fname, "w", encoding="utf-8") as f:
                f.write(text)
            sent = await client.send_document(chat_id, fname, caption="Transcription File", reply_to_message_id=reply_id)
            try: os.remove(fname)
            except: pass
            return sent
...
def index():
    ...
    return render_template_string("Bot is running. Uptime: {{u}}s", u=uptime)
