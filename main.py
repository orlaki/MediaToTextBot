...
def lang_cb(call):
    ...
    try:
        text = transcribe_with_speech_recognition(file_path, language=code)
        if not text:
...
def handle_media(message):
    ...
    try:
        ...
        if not lang:
            ...
            bot.reply_to(message, "Select the language spoken in your audio or video:", reply_markup=kb)
            return
        text = transcribe_with_speech_recognition(file_path, language=lang)
        if not text:
...
def transcribe_with_speech_recognition(input_path, language=None):
    recognizer = sr.Recognizer()
    chunk_dir = tempfile.mkdtemp(prefix="chunks_", dir=DOWNLOADS_DIR)
    ...
    chunks = sorted([os.path.join(chunk_dir, f) for f in os.listdir(chunk_dir) if f.lower().endswith(".wav")])
    if not chunks:
        if os.path.exists(chunk_dir):
            shutil.rmtree(chunk_dir, ignore_errors=True)
    ...
    for ch in chunks:
        try:
            with sr.AudioFile(ch) as source:
