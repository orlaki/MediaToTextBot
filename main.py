...
OFFLINE_RECOGNIZER = os.environ.get("OFFLINE_RECOGNIZER", "false").lower() in ("1", "true", "yes")
        ...
        for i in range(0, total_ms, MAX_CHUNK_MS):
            ...
            try:
                if OFFLINE_RECOGNIZER:
                    try:
                        if language:
                            part = recognizer.recognize_sphinx(audio_data, language=language)
                        else:
                            part = recognizer.recognize_sphinx(audio_data)
                    except AttributeError:
                        raise RuntimeError("Offline recognizer not available")
                else:
                    if language:
