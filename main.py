...
async def send_key_missing_alert(chat_id):
    try:
        if TUTORIAL_CHANNEL:
            pinned = await bot.get_chat(TUTORIAL_CHANNEL)
            if getattr(pinned, "pinned_message", None):
                await bot.forward_messages(chat_id, TUTORIAL_CHANNEL, pinned.pinned_message.message_id)
    except:
        pass
...
async def text_handler(client, message):
    ...
    txt = message.text or ""
    if user_awaiting_key.get(uid) and not txt.startswith("/"):
        key = txt.strip()
        if not is_gemini_key(key):
...
async def callback_query_handler(client, query: CallbackQuery):
    ...
    if data.startswith("lang|"):
        try:
            await client.edit_message_reply_markup(query.message.chat.id, query.message.message_id, reply_markup=None)
        except:
            pass
        parts = data.split("|")
        if len(parts) >= 4:
            _, code, lbl, origin = parts[0], parts[1], parts[2], parts[3]
        else:
            return
        await process_text_action(query, origin, f"Translate to {lbl}", f"Translate this text in to language {lbl}. No extra text ONLY return the translated text.")
        return
    if data.startswith("translate_menu|") or data.startswith("summarize|"):
        action = data.split("|")[0]
        if action == "translate_menu":
            try:
                await client.edit_message_reply_markup(query.message.chat.id, query.message.message_id, reply_markup=build_lang_keyboard("trans"))
            except:
                pass
        else:
            try:
                ...
                pass
            await process_text_action(query, query.message.message_id, "Summarize", "Summarize this in original language.")

async def process_text_action(query_or_call, origin, log_action, prompt_instr):
    if isinstance(query_or_call, CallbackQuery):
        call = query_or_call
        ...
        return
    try:
        data = user_transcriptions.get(chat_id, {}).get(int(origin))
    except:
        data = None
    if not data:
        ...
        return
    text = data["text"]
    key = f"{chat_id}|{origin}|{log_action}"
    ...
    try:
        ...
        res = await loop.run_in_executor(None, ask_gemini, text, prompt_instr, from_user_id)
        with lock:
            action_usage[key] = action_usage.get(key, 0) + 1
        persist_action_usage_to_db(key)
        await send_long_text(chat_id, res, data["origin"], from_user_id, log_action)
        ...
        if msg == "API_KEY_MISSING":
            ...
            parts = msg.split("|")
            secs = int(parts[1]) if len(parts) > 1 else seconds_left_for_user(from_user_id)
            await bot.send_message(chat_id, f"Daily limit reached. Time left: {format_hms(secs)}.")
        else:
            await bot.send_message(chat_id, f"Error: {e}")

@bot.on_message(filters.voice | filters.audio | filters.video | filters.document)
async def handle_media(client, message):
    ...
    file_path = os.path.join(DOWNLOADS_DIR, f"temp_{message.message_id}_{getattr(media, 'file_unique_id', '')}")
    try:
        downloaded_path = await message.download(file_path)
        loop = asyncio.get_running_loop()
        try:
            text = await loop.run_in_executor(None, upload_and_transcribe_gemini, downloaded_path, message.from_user.id)
        except Exception as e:
            em = str(e)
        ...
        sent = await send_long_text(message.chat.id, text, message.message_id, message.from_user.id)
        if sent:
            sent_id = sent.message_id
            user_transcriptions.setdefault(message.chat.id, {})[sent_id] = {"text": text, "origin": message.message_id}
            try:
                await bot.edit_message_reply_markup(message.chat.id, sent_id, reply_markup=build_action_keyboard(len(text)))
            except:
                pass
    except Exception as e:
...
async def send_long_text(chat_id, text, reply_id, uid, action="Transcript"):
    if len(text) > MAX_MESSAGE_CHUNK:
        ...
        with open(fname, "w", encoding="utf-8") as f:
            f.write(text)
        sent = await bot.send_document(chat_id, fname, caption="Open this file and copy the text inside 👍", reply_to_message_id=reply_id)
        try:
            os.remove(fname)
        except:
            pass
        return sent
    return await bot.send_message(chat_id, text, reply_to_message_id=reply_id)
