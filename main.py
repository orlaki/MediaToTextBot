...
def convert_to_wav(input_path: str) -> str:
    ...
    output_path = os.path.join(DOWNLOADS_DIR, f"{base}_converted.wav")
    command = [FFMPEG_BINARY, "-y", "-i", input_path, "-acodec", "pcm_s16le", "-ac", "1", "-ar", "16000", output_path]
    try:
        ...
        raise RuntimeError(f"ffmpeg failed: {stderr}") from None
    ...
    def perform_upload_and_transcribe(key):
        ...
        try:
            ...
            with open(file_path, 'rb') as f:
                up_resp_raw = requests.post(upload_url, headers=headers, data=f.read(), timeout=REQUEST_TIMEOUT_GEMINI)
            try:
                up_resp = up_resp_raw.json()
            except Exception:
                raise RuntimeError("Upload response not JSON")
            ...
            if not uploaded_name and not uploaded_uri:
                raise RuntimeError("Upload failed or returned no file reference")
    ...
    try:
        ...
        if not file_ext:
            ...
            if 'ogg' in mime:
                ...
                file_ext = ''
        file_path = os.path.join(DOWNLOADS_DIR, f"temp_{message.id}_{media.file_unique_id}{file_ext}")
