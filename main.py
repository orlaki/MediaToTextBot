...
def get_user_usage_count(uid):
    return user_usage_memory.get(uid, 0)

def increment_user_usage_count(uid):
    user_usage_memory[uid] = user_usage_memory.get(uid, 0) + 1
    return user_usage_memory[uid]
...
async def handle_media(client, message: Message):
    ...
    if sent_message:
        try:
            ...
            if REQUIRED_CHANNEL:
                current_c = increment_user_usage_count(uid)
                if current_c == USER_FREE_USAGE:
