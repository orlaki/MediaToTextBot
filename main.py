...
model = WhisperModel(model_size_or_path=WHISPER_MODEL_SIZE, device=WHISPER_DEVICE, compute_type=WHISPER_COMPUTE_TYPE)
...
def lang_cb(call):
    ...
    try:
        text = transcribe_faster_whisper(file_path, language=code)
        if not text:
...
def handle_media(message):
    ...
    try:
        ...
        if not lang:
            ...
            bot.reply_to(message, "Select the language spoken in your audio or video:", reply_markup=kb)
            return
        text = transcribe_faster_whisper(file_path, language=lang)
        if not text:
...
def transcribe_faster_whisper(file_path, language=None):
