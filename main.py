...
def lang_cb(call):
    ...
    try:
        text = transcribe_local(file_path, language=code)
        if not text:
...
def handle_media(message):
    ...
    try:
        ...
        if not lang:
            ...
            bot.reply_to(message, "Select the language spoken in your audio or video:", reply_markup=kb)
            return
        text = transcribe_local(file_path, language=lang)
        if not text:
...
def transcribe_local(input_path, language=None):
    base = os.path.splitext(os.path.basename(input_path))[0]
    conv_wav = os.path.join(DOWNLOADS_DIR, f"{base}_conv.wav")
    try:
        subprocess.run(["ffmpeg", "-y", "-i", input_path, "-ar", "16000", "-ac", "1", conv_wav], check=True, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
    except Exception as e:
        raise RuntimeError(f"ffmpeg conversion failed: {e}")
    duration = 0.0
    try:
        with wave.open(conv_wav, "rb") as wf:
            frames = wf.getnframes()
            rate = wf.getframerate()
            duration = float(frames) / float(rate) if rate else 0.0
    except Exception:
        duration = 0.0
    chunks = []
    if duration > 60.0:
        chunk_dir = os.path.join(DOWNLOADS_DIR, f"chunks_{int(time.time()*1000)}")
        os.makedirs(chunk_dir, exist_ok=True)
        pattern = os.path.join(chunk_dir, "chunk_%03d.wav")
        try:
            subprocess.run(["ffmpeg", "-y", "-i", conv_wav, "-ar", "16000", "-ac", "1", "-f", "segment", "-segment_time", "60", pattern], check=True, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
    ...
    try:
        if os.path.exists(conv_wav) and conv_wav not in chunks:
            os.remove(conv_wav)
        for c in chunks:
            try:
                if os.path.exists(c):
                ...
                pass
        chunk_parent = os.path.dirname(chunks[0]) if chunks and len(chunks) > 1 else None
        if chunk_parent and os.path.isdir(chunk_parent):
            try:
                shutil.rmtree(chunk_parent)
            except:
                pass
    except:
        pass
