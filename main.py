...
async def handle_media(client, message: Message):
    ...
    if sent_message:
        ...
        try:
            new_count = increment_user_usage_count(uid)
            if REQUIRED_CHANNEL and new_count >= USER_FREE_USAGE:
                try:
                    joined = await is_user_in_channel(client, uid)
                    if not joined:
                        await send_join_prompt_to_target(client, uid, reply_target=sent_message)
                except Exception:
                    try:
                        await client.send_message(uid, f"🚫 You have reached your free limit of {USER_FREE_USAGE}. Please join {REQUIRED_CHANNEL} to continue.")
                    except Exception:
                        pass
